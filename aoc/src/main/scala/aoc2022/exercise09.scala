...
object exercise09 extends App:
  ...
  object Grid:
    ...
    def followHead(g: Grid): Grid =
      ...
      val newTail = g
                .allTails
                .scanLeft(g.h)((x,y) => inner(x,y))
                .tail // only take tail don't want the g.h

      g.copy(allTails = newTail,
            visited = g.visited.concat(List(newTail.reverse.head))) // should be easier way to get newest
